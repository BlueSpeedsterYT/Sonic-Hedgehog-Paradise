/// @description  player_state_spindash();
	 ...
	 // Trigger rolling if we're spindashing and no longer holding down:
	    if(state == STATE_SPINDASH)
	    {
	       // Change to the rolling state and change the horizontal speed:
	          ...
	          {
	             if((AnimationDirection == 1 && (!player_collision_check(COL_RIGHT, MASK_BIG, x, y, angle))) || (AnimationDirection == -1 && (!player_collision_check(COL_LEFT, MASK_BIG, x, y, angle))) )
	             {
	                state              = STATE_ROLL;
	                x_speed            = AnimationDirection*(8+spindash_strength);              
