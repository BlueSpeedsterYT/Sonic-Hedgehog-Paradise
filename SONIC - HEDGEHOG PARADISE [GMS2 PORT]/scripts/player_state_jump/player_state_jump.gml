/// @description  player_state_jump();
function player_state_jump() {
	 // The players jumping state.
     
	 // Variable jump:
	    if(input_a == false && y_speed < jump_release && state == STATE_JUMP && jump_completed = false)
	    {
	       y_speed = jump_release;
	    }

	 // Full jump:
	    if((ground == true || state == STATE_CORKSCREW || state == STATE_CORKSCREW_ROLL) && !player_collision_check(COL_TOP, MASK_BIG, x, y-6, angle) && input_a_pressed)
	    {
